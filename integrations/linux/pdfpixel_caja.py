"""Caja shim — dumb Popen of the pdfpixel CLI. All UI (range dialog,
notification) lives in the CLI process, so this never blocks Caja."""
import os
import shutil
import subprocess
...
class PdfPixelExtension(GObject.GObject, Caja.MenuProvider):
    ...
    def get_file_items(self, *args):
        # Caja 4.0 passes (files,); older nautilus-python passes (window, files).
        files = args[-1]
